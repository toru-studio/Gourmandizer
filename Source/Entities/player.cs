...
public partial class player : CharacterBody2D
{
	...
	private float _currentWeight = 5.0f;
	private int _foodItems = 0;
	private PackedScene _foodScene = GD.Load<PackedScene>("res://Source/Item/WearableFood.tscn");
	...
	private void AttachFood()
	{
	
		var joint = (PinJoint2D)GetChild(5);
		joint.Position = new Vector2(100, 100);
		joint.Softness = 0.5f;
		
		
		var food = new WearableFood();
		food.Position = new Vector2(50, 0);
		Console.WriteLine(food.Name);
		this.AddChild(food);
		joint.NodeB = food.GetPath();
	}
}
